...
\begin{frame}[fragile] {Registreringsoversikt}
<<'TabReg', results='asis', echo=FALSE, eval=T>>=
tabAntOpph <- tabAntOpphShMnd(RegData=RegData, datoTil=datoTil, reshID = reshID, antMnd=12)
...
<<'TabNokkeltall', results='asis', echo=FALSE, eval=T>>=
tabNokkeltall <- tabNokkeltall(RegData=RegDataEget, tidsenhet='Kvartal', datoTil=datoTil, enhetsUtvalg=2, reshID=reshID)
xtable(tabNokkeltall, digits=0, align=c('l', rep('r', ncol(tabNokkeltall))),
		caption=paste0('Nøkkeltall, ', shtxt,'.'), label='tab:NokkeltallEget')
@
\end{frame}
...
dum <- RyggFigAndelTid(RegData=RegData, preprosess=0, valgtVar='ohjelp',
                       enhetsUtvalg = 1, reshID = reshID, tidsenhet = 'Kvartal',
                       outfile='Ohjelp.pdf')

dum <- RyggFigAndeler(RegData=RegData, preprosess=0, valgtVar='inngrep', enhetsUtvalg = 1,
...
\begin{figure}[ht]
\centering
\includegraphics[scale=0.35]{ventetidPoliklin.pdf}
...
\item Andel Ø-hjelp, (egen sammenlignet med resten), kvartalsvis MÅ LAGES
\item Inngrepstype
\item Postoperativ liggetid prolaps (egen sammenlignet med resten), kvartalsvis
\item Postoperativ liggetid spinal stenose (egen sammenlignet med resten), kvartalsvis
\item Symptomvarighet

...
RegData <- RyggRegDataV2V3(datoFra = datoFra) #, datoTil = datoTil)
RegData <- RyggPreprosess(RegData=RegData)

shtxt <- RegData$SykehusNavn[match(reshID, RegData$ReshId)]
...
RegDataEgetMnd <- RegDataEget6mnd[which(as.Date(RegDataEget6mnd$OpDato, tz='UTC') >= datoFraMnd), ]
RegDataResten6mnd <- RegData6mnd[setdiff(1:N,ind$Hoved), ]

maxDatoEget <- format.Date(max(RegDataEget6mnd$OpDato), format='%d.%m.%Y', tz='UTC')
...
xtable::xtable(tabAntOpph, digits=0, align=c('l', rep('r', ncol(tabAntOpph))),
...
xtable::xtable(tabNokkeltall, digits=0, align=c('l', rep('r', ncol(tabNokkeltall))),
